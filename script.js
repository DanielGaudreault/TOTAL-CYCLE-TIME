...
function updateToExcel() {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Sum up cycle times for each project name
            let cycleTimeSums = {};
            ...
            excelRows.forEach(row => {
                const projectName = row[1]?.toString().trim(); // 'Project Name' is in column B (index 1)
                if (projectName in cycleTimeSums) {
                    row[3] = cycleTimeSums[projectName]; // Update cycle time in column D (index 3)
                    console.log(`Updated cycle time for project ${projectName}: ${cycleTimeSums[projectName]}`);
                }
            });
